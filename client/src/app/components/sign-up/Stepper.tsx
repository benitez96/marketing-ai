...
export const Stepper = ({ steps, currentStep }: any) => {
    ...
    const updateStep = (stepNumber: any, steps: any): any => {
        ...
        return (
            <div key={index} className={index !== newStep.length - 1 ? 'w-full flex items-center' : "flex items-center"}>
                <div className='relative flex flex-col items-center text-teal-600'>
                    <Button
                        radius="full"
                        color={step.selected ? 'secondary' : 'default'}
                        className={`rounded-full transition duration-500 ease-in-out border-2 border-white h-12 w-12 flex items-center justify-center py-3`}>
                        {/* Display Number */}
                ...
                <div className={`flex-auto border-t-2 transition duration-500 ease-in-out ${step.completed ? "border-black" : "border-gray-300"}`}>
                </div>
            </div>
        )
    })
